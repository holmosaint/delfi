...
class ContainerSelfFilter(ContainerFilter):
    def __init__(self, tau_func, dt=DT):
        ...
        def step(variables, parameters):
            d = -1*self.variables['v']+parameters["s"]
            variables["v"] += self.dt*d/max(float(tau_func(parameters["s"],
                                                           self)), self.dt)
            return variables
        self.container = Container(self.variables, step)
...
class AverageFilter:
    ...
    def filter(self, trace):
        ...
        for i in range(trace.shape[0]):
            d = trace[max(0, int(i-self.n_window/2)):
                      int(i+self.n_window/2)]
            averaged[i] = d.mean()
        return averaged
...
class MiddleFilter(FunctionTemporalFilter):
    ...
    def __init__(self, tau, c, step, dt=DT, keep_head=True):
        tau = float(tau)
        func = lambda x: np.exp(-x/tau)/tau-np.exp(-x/(c*tau))/(c*tau)
...
class SlowFilter(FunctionTemporalFilter):
    def __init__(self, tau, step, dt=DT):
        ...
        FunctionTemporalFilter.__init__(self, func, step, dt,
...
class ConvolutionKernelFilter(FunctionTemporalFilter):
    ...
    """
    def __init__(self, tau_r=0.05e3, tau_d=0.05e3, tau_phase=150,
                 theta=-np.pi/7,
                 r=1., step=250, dt=DT):
        def func(x):
            y = -(x/(r*tau_r)**3)/(1+x/(r*tau_r))
            y *= np.exp(-(x/(r*tau_d))**2)
            y *= np.cos(2*np.pi*x/(r*tau_phase)+theta) # fixed from paper
            return y
        FunctionTemporalFilter.__init__(self, func, step, dt,
                                        normalize_weight=True)
...
class EPSCFilter:
    ...
    def filter(self, trace):
        ...
        for i, release in enumerate(trace):
            if t_mepsc+i > t_epsc:
                EPSC[i:i+t_mepsc] += release*self.mepsc[:t_epsc-i-t_mepsc]
            else:
...
class TauEPSCFilter(EPSCFilter):
    def __init__(self, A=0.002, raise_tau=0, decay_tau=4, K=4, dt=DT):
        if raise_tau == 0:
            ...
            weights = [A*(1-np.exp(-dt*i/raise_tau)) for i in range(int(raise_tau*K/dt))]
        weights += [weights[-1]*np.exp(-dt*i/decay_tau) for i in range(int(decay_tau*K/dt))]
        weights = np.array(weights)
