...
def get_step_stimulus(pre_time=50000,
                      stage_time=30000,
                      stages=np.arange(0.1, 5, 0.5),
                      dt=DT):
...
def get_noise_stimulus(mean_stim,
                       std_stim,
                       mean_steptime,
                       std_steptime,
                       test_time,
    ...
    if type(n_pool) == bool and n_pool:
        ...
        if need_init:
            if single_block is None:
                model_resps = [
                    model.run(init_stims[i], traces4evaluates[i])[0]
            ...
            if single_block is None:
                model_resps = [
                    model.run(j)[0]
                    for i, j in enumerate(traces4evaluates)
                    if i % nprocs == rank
                ]
            else:
                model_resps = [
        ...
        if single_block is None:
            valid_resps = [
                j for i, j in enumerate(valid_responses) if i % nprocs == rank
            ]
        else:
            valid_resps = [
                j for i, j in enumerate(valid_responses)
                if i % single_block == rank % single_block
            ]
        if erase_resting:
        ...
        if need_init:
            model_resps = [
                model.run(init_stim, trace)[0]
                for init_stim, trace in zip(init_stims, traces4evaluates)
            ]
        else:
    ...
    def func(x):
        ...
        if type(n_pool) is bool and n_pool:
            ...
            if single_block is None:
                ...
                if rank % single_block == 0:
                    ...
                    value = comm.recv(source=rank - (rank % single_block),
                                      tag=3)
            comm.Barrier()
        v = float(value)
        if record_fname is None:
            ...
            if single_block is not None and rank % single_block != 0:
                return v
            if type(n_pool
                   ) is bool and n_pool and single_block is None and rank != 0:
                return v
            if nprocs > 1:
