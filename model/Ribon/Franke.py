...
[15.67777320402249, -0.056791206795922054, 1.1191557005173172, 10.822462928458327, 128.08111364345706, 0.06406046475305106, 0.6935847271389265, 0.058909332828580865, 8.98137607778845e-05, 51.656473505260884, 19.826843632437477, 0.051296586456530686, 132.0951533495297]

[0.2867, 1.4255,
 9.616,
 0.067, 0.73, -0.6, 175.,
 9.74729,
 0.061, 0.00012, 0.0377,
 50.12,
 0.00188, 2., 5., 70.]

[3.433325684022069, 4.566643659008289, 14.499384645924543, 0.3753353767148771, 0.43221318387868524, -5.130981624566903, 162.71439042342692, 20.368198422735595, 0.357587436146311, 0.00043905120636321036, 0.07224261102611453, 308.7228832049189, 0.009774450357724025, 30.045522639775164, 2.530232150130778, 1071.3366307485592]
[1.82042759508533, 4.652947158285871, 8.202554350214859, 0.39982186424442145, -0.20966383531977761, -3.249582221496651, 242.6780543380264, 4.069556208192722, 0.5424813331526837, 0.0007236331063469786, 0.057927785290579376, 359.48174076620444, 0.006837660918257103, -31.891726303407516, 6.045331127525978, 236.90869522818718]
[3.244723254660908, 10.915303386957811, 13.878967141197133, 0.06062008599349561, -0.4843435671050944, -3.776928715874104, 1630.118166390761, 13.132666823390942, 0.44096305505105343, 0.0009099725948107005, 0.007782585551281132, 23.414101693288195, 0.009519029863395442, -29.208232385298352, 11.509424020391268, 3306.121801011765]
...
class SinglePathwayModel:
    ...
    """
    def __init__(self, parameters, dt=DT, feedback_mode=0, RMAX=False,
                 single=False):
        ...
        if single:
            ...
            if RMAX:
                gain_infi1 = get_func_from_parameters("rhill", [parameters[4]]+[1.])
                self.ribbon1 = RmaxRibbon(1./parameters[5], gain_infi, RRP=1./dt, dt=dt)
            else:
                gain_infi1 = get_func_from_parameters("rhill", parameters[4:6])
                gain_tau1 = get_func_from_parameters("sigmoid", parameters[6:9])
                self.ribbon1 = Ribbon(1., gain_infi1, gain_tau1, RRP=1./dt, dt=dt)
        else:
            self.sigmoid1 = get_func_from_parameters("sigmoid", list(parameters[:2])+[1.])
            self.temporal1 = ContainerFilter(lambda x: parameters[2], dt=dt)
            if RMAX:
                gain_infi1 = get_func_from_parameters("rhill",
                                                      [parameters[3]]+[1.])
                self.ribbon1 = RmaxRibbon(1./parameters[4], gain_infi1, RRP=1./dt, dt=dt)
                self.temporal2 = ContainerFilter(lambda x: parameters[5], dt=dt)
                self.sigmoid2 = get_func_from_parameters("sigmoid",
                                                         parameters[6:9])
                self.temporal3 = ContainerFilter(lambda x: parameters[9],
                                                 dt=dt)
                gain_infi2 = get_func_from_parameters("rhill",
                                                      [parameters[10]]+[1.])
                self.ribbon2 = Ribbon(1./parameters[11], gain_infi2, RRP=1./dt, dt=dt)
                self.temporal4 = ContainerFilter(lambda x: parameters[12],
                                                 dt=dt)
            else:
                gain_infi1 = get_func_from_parameters("rhill",
                                                      parameters[3:5])
                gain_tau1 = get_func_from_parameters("sigmoid",
                                                     parameters[5:8])
                self.ribbon1 = Ribbon(1., gain_infi1, gain_tau1,
                                      RRP=1./dt, dt=dt)
                ...
                gain_tau2 = get_func_from_parameters("sigmoid",
                                                     parameters[15:18])
                self.ribbon2 = Ribbon(1., gain_infi2, gain_tau2,
                                      RRP=1./dt, dt=dt)
    ...
    def run(self, trace):
        ...
        if self.fb_mode == 0:
            ...
            return ribbon2+[temporal3, sigmoid2,
                            temporal2]+ribbon1+[temporal1, sigmoid1]
        if self.fb_mode == 1:
...
def get_stimulus(DATATYPE=0):
    if DATATYPE == 0:
        stim = np.loadtxt('./Data/chirp_stim.txt')
    elif DATATYPE == 1:
        stim = np.loadtxt('./Data/DA18_normalized_0.1.txt')
    return stim
...
def save_response(prefix="./Data/lchirp_control_resp",
                  label='lchirp_avg'):
    for i in range(1, 15):
...
def get_data_pair_lchirp(cluster_id, buffer_time=10000, DATATYPE=0, dt=DT):
    ...
    response = np.loadtxt("./Data/lchirp_control_resp_%d.txt"%cluster_id)
    stim = stim[::int(dt/DT)]
...
def fit_CC(cluster_id, x0=None, ext_model_parameters={'fixed_parameters': []},
           DATATYPE=0, dt=DT):
    pre_step, stim, resp = get_data_pair_lchirp(cluster_id,
                                                DATATYPE=DATATYPE, dt=dt)
    import nr
    MODEL = nr.NTGCNTGC
    if x0 is None:
        p_base = [0.01, -1, 50, 50, 0.01, 0.01, 0.01, 0, 0.1, 50, 50, 0.01, 0.01]
        p_range = [1, 2, 500, 500, 2, 2, 1, 1, 2, 500, 500, 2, 2]
        x0 = np.random.random(len(p_base))*np.array(p_range)+np.array(p_base)
    return fit(MODEL, x0, False, cluster_id,  (pre_step, stim, resp),
                             ext_model_parameters, dt)

def fit(MODEL, x0, need_init, cluster_id, data, ext_model_parameters={},
        fname=None, dt=DT):
