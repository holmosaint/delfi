...
def sliding_window(X, Y, resting_input, resting_response, 
                   run_step, buffer_step, pre_step):
    run_begin = pre_step+buffer_step
    all_time = X.shape[1] - X.shape[1]%run_step # cut tail
    input_traces = [np.zeros((X.shape[0], run_begin+run_step))
                    for i in range(0, all_time, run_step)]
    output_traces = [np.zeros(run_begin+run_step)
                     for i in range(0, all_time, run_step)]
    ...
    for i, index_begin in enumerate(range(0, all_time, run_step)):
        input_traces[i][:, run_begin:] = X[:, index_begin:index_begin+run_step]
        ...
        if i > 0:
            ...
            input_traces[i][:, run_begin-buffer_length:run_begin] = X[:, index_begin-buffer_length:index_begin]
