...
def sym_sig_inv(x):
    ...
    return tt.log(x /
                  (1. - x))  # there's probably more stable implementations...


def sig(x):

    return 1. / (1. + np.exp(-x)
                )  # there's probably more stable implementations...


class BoxConstraintConditionalAutoregressiveFlow(
    ...
    def __init__(self,
        ...
        L = theano.clone(output=self.L,
                         replace={self.y: y_sig},
                         share_inputs=True)
        self.L = L - tt.sum(tt.log(self.diff * y_scaled * (1. - y_scaled)),
                            axis=1)
        self.L.name = 'L'
    ...
    def gen(self,
            ...
            n_samples=None,
            u=None,
            rng=np.random):  # overwrite that one
